...
import { ArrowLeft, Check, Eye, EyeOff, Play, Plus } from "lucide-react";
...
/**
 * Wort-Nachschlag. Übernehmen geht über den Pool, wenn es das Lemma dort gibt;
 * sonst als eigene Karte — beim Lesen begegnen einem Wörter, die der kuratierte
 * Pool nicht führt, und daran soll der Lernfluss nicht enden.
 */
function GlossSheet({
  token,
  gloss,
  ...
  async function add() {
    ...
    try {
      ...
      if (result.added) {
        onAdded(gloss.lemma);
        toast.success(T.stories.gloss.added);
      }
      onClose();
    } finally {
      setBusy(false);
      ...
      {gloss && (
        <div className="space-y-4">
          <div>
            ...
            {gloss.proper ? (
              <p className="mt-1 text-xs text-muted-foreground">{T.stories.gloss.properName}</p>
            ) : (
              gloss.lemma !== token && (
                <p className="mt-1 text-xs text-muted-foreground">
                  {T.stories.gloss.baseForm(gloss.lemma)}
                </p>
              )
            )}
            {isNew && (
          ...
          {gloss.structure && (
            <p className="rounded-xl border border-dashed border-border bg-muted/30 p-3 text-xs text-muted-foreground">
              {T.stories.gloss.structure}
            </p>
          )}

          <button
            type="button"
            aria-label={T.stories.gloss.listenAria}
          ...
          {addable ? (
            ...
            >
              <Plus className="h-4 w-4" /> {T.stories.gloss.addCard}
            </button>
          ) : (
            !gloss.proper &&
            !gloss.structure && (
              <p className="text-xs font-medium text-muted-foreground">{T.stories.gloss.inCards}</p>
            )
          )}
        </div>
      )}
