...
// In-Memory-Map ersetzt. Reicht: die Module nutzen nur get/set/clear.
...
function seedV1(vocab: VocabEntry[], stats?: Partial<UserStats>) {
  db.set("vocab:list", vocab);
  if (stats) db.set("stats:current", stats);
}
...
describe("Migration v1 → v2", () => {
  ...
  it("schützt Bestandskarten ab Box 4 als gefestigt", async () => {
    ...
    await runMigrations();
    const list = db.get("vocab:list") as VocabEntry[];
  ...
  it("gibt Bestandsnutzern einen Willkommens-Joker und leere Wochentage", async () => {
    ...
    await runMigrations();
    const stats = db.get("stats:current") as UserStats;
